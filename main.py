...
def info() -> typing.Dict:
    ...
    return {
        "apiversion": "1",
        "author": "Gruppe 17",  # My Battlesnake Username
...
def eval_state(my_head, my_body, opponents, food_list, board_width,
               board_height):
    if not food_list:  # If there is not food on the board
        return 0
    # Find the move that brings the snake closest to the nearest food
    nearest_food = min(food_list, key=lambda f: manhattan(my_head, f))
    dist_tofood = manhattan(my_head, nearest_food)
    ...
    danger = sum(  # If the opponent is close and bigger than you, it is dangerous
        1 for snake in opponents if manhattan(my_head, snake["body"][0]) == 1
...
def minimax(game_state, depth, alpha, beta, maximizing_player):
    ...
    if depth == 0 or not food_list:  # If the maximum depth is reached or there is no food on the board
        return eval_state(my_head, my_body, opponents, food_list, board_width,
                          board_height), None
    ...
    if maximizing_player:  # Maximizing player
        ...
        for move, new_head in direction.items():
            if not is_safe(new_head, my_body, opponents, board_width,
                           board_height):  # If the move is not safe, skip it
                continue

            new_body = [new_head
                        ] + my_body[:-1]  # Update the body of the snake
            new_state = copy.deepcopy(
                game_state)  # Create a copy of the game state
            new_state['you'][
                'body'] = new_body  # Update the body of the snake in the new game state
            new_state["you"]["health"] -= 1  # Decrease the health of the snake
            eval, _ = minimax(new_state, depth - 1, alpha, beta,
        ...
        for move, new_head in direction.items():
            ...
            min_eval = min(min_eval,
                           eval)  # Update the minimum evaluation score
...
def move(game_state: typing.Dict) -> typing.Dict:
    _, best_move = minimax(game_state, MAX_DEPTH, float('-inf'), float('inf'),
                           True)  # Call the minimax algorithm
    if not best_move:
        best_move = random.choice(
            ["up", "down", "left",
             "right"])  # If no best move is found, choose a random move
